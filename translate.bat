...
dub run dpp@0.5.5 --build=release -- --ignore-cursor=stat64 --ignore-cursor=PyType_HasFeature --ignore-cursor=_Py_IS_TYPE  --ignore-cursor=_PyObject_TypeCheck --ignore-cursor=COMPILER --ignore-cursor=_PyCode_DEF --ignore-cursor=Py_IS_TYPE --function-macros --preprocess-only --include-path "%PYTHON_INCLUDE_PATH%" "%IFILE%"
